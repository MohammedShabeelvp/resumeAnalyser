    ...
    <div className="card p-5">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          {renaming ? (
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={() => setRenaming(true)} className="text-[var(--color-muted)] hover:text-[var(--color-teal)]">
          ...
          <p className="truncate text-xs text-[var(--color-muted)]">{item.filename}</p>
        ...
        <div className="flex shrink-0 flex-col items-end gap-2">
          ...
          <button onClick={handleDelete} className="text-[var(--color-muted)] hover:text-red-600">
        ...
        {!expanded ? (
          ...
          <div>
            ...
            <div className="mt-2 flex gap-2">
              ...
              <button onClick={() => setExpanded(false)} className="text-xs text-[var(--color-muted)]">
                Cancel
              </button>
            </div>
          </div>
    ...
    <div className="page-container py-12">
        ...
        {items.length > 0 &&
          (confirmingClear ? (
            <div className="flex items-center gap-2 text-sm">
              <span className="text-[var(--color-muted)]">Delete all history?</span>
              <button onClick={handleClearAll} className="text-red-600">
                Confirm
              </button>
              <button onClick={() => setConfirmingClear(false)} className="text-[var(--color-muted)]">
                Cancel
              </button>
            </div>
          ) : (
      ...
      {history.isSuccess && items.length === 0 && (
        <div className="card p-10 text-center">
          <svg width="48" height="48" viewBox="0 0 48 48" fill="none" className="mx-auto mb-4 text-[var(--color-muted)]">
            <circle cx="24" cy="24" r="16" stroke="currentColor" strokeWidth="1.5" />
            <path d="M24 16v8l6 3" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
          </svg>
          <p className="mb-4 text-sm text-[var(--color-muted)]">No analyses yet. Upload a resume to get started.</p>
