  ...
  return (
    <div className="page-container py-12">
      ...
      <div className="card mb-6 p-6" style={{ animation: "slideUp 0.4s ease both", ...delays[4] }}>
        ...
        {gaps.priority && gaps.priority.length > 0 && (
          <div className="mb-4 rounded-lg p-3" style={{ background: "#FFFBEB" }}>
            <p className="mb-2 text-xs" style={{ color: "#92400E" }}>
              Priority gaps
            </p>
            <div className="flex flex-wrap gap-1.5">
              {(Array.isArray(gaps.priority) ? gaps.priority : [gaps.priority]).map((item) => (
                <SkillBadge key={item} skill={item} variant="missing" />
              ))}
            </div>
          </div>
        )}
