    ...
    <div className="page-container py-12" style={{ animation: "fadeIn 0.3s ease" }}>
      ...
      <section className="card mb-6 max-h-48 overflow-y-auto p-5">
        ...
        <div className="flex flex-wrap gap-2">
          {allSkills.length === 0 ? (
            <p className="text-sm text-(--color-muted)">No skills detected.</p>
          ) : (
      ...
      <section className="card mb-6 p-5">
        ...
        {recommend.isLoading && <p className="text-sm text-(--color-muted)">Finding roles that fit…</p>}
        {recommend.isError && <p className="text-sm text-red-600">{recommend.error}</p>}
        {recommend.isSuccess && (
          <div className="max-h-60 space-y-2 overflow-y-auto">
            {(recommend.data?.recommendations || []).map((rec) => (
              <div key={rec.role} className="flex items-center justify-between border-b border-(--color-border) py-2 last:border-none">
                <div>
                  <p className="text-sm font-medium">{rec.role}</p>
                  <p className="text-xs text-(--color-muted)">{rec.reason}</p>
                </div>
      ...
      <section className="card mb-6 p-5">
        ...
        {!user && (
          <p className="mt-2 text-center text-[10px] text-(--color-muted) opacity-70">
